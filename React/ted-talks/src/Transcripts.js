    ...
    <div className="tagText">
      ...
      <PieChart width={1200} height={400}>
        <Pie data={transcripts} cx={250} cy={200} innerRadius={80} outerRadius={160} fill="#8884d8" label>
          {
            transcripts.map((entry, index) => <Cell key={entry} fill={colors[index % colors.length]}/>)
          }
        </Pie>
        <Tooltip/>
      </PieChart>
      ...
      <div>
      Once we selected the number of words to used, we used a linear algebra technique called Singular Value Decomposition to determine
      the underlying shape of each talk. The first three singular vectors are discussed below. They represent different components
      of the plot line of each talk.
      </div>
      <div className="space"/>
...
export default shouldUpdate(checkPropsChange)(Transcripts);